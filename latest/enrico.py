...
layout = html.Div([
    ...
    html.H3('Data Policy'),
    html.Div([
        html.Div('Policy Encrypting Key: ', className='two columns'),
...
@app.callback(
    ...
    [Input('gen-measurements-update', 'n_intervals'),
     Input('generate-button', 'n_clicks')],
    [State('generate-button', 'n_clicks_timestamp'),
     State('policy-pub-key', 'value'),
     State('cached-last-readings', 'children')],
)
def generate_vehicular_data(n, n_clicks, gen_time, policy_pubkey_hex, cached_last_reading):
    if gen_time == 0:
    ...
    if not cached_data_source:
        ...
        data = {
            'data_source_ver_key': data_source_verifying_key,
        }
