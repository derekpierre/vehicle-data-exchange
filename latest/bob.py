...
@app.callback(
    ...
    [Input('gen-key-button', 'n_clicks')],
    [State('bob-unique-id', 'children')]
)
def get_bob_keys(n_clicks, bob_id):
    if n_clicks > 0:
        bob_pubkeys = demo_keys.get_recipient_pubkeys(bob_id)
        return html.Div([
            html.Div([
                html.Div('Encryption Key:', className='two columns'),
                html.Div(f"{bob_pubkeys['enc'].to_bytes().hex()}", className='seven columns'),
            ], className='row'),
            html.Div([
                html.Div('Verifying Key:', className='two columns'),
                html.Div(f"{bob_pubkeys['sig'].to_bytes().hex()}", className='seven columns')
            ], className='row')
        ])
...
@app.callback(
    ...
    [Input('measurements-update', 'n_intervals'),
     Input('read-button', 'n_clicks')],
    [State('read-button', 'n_clicks_timestamp'),
     State('latest-decrypted-measurements', 'children'),
     State('bob-unique-id', 'children')]
)
def update_cached_decrypted_measurements_list(n, read_n_clicks, read_time, df_json_latest_measurements, bob_id):
    if read_n_clicks == 0 or read_time == 0:
    ...
    if not joined:
        print("The Insurer joins policy for label '{}' "
              "and pubkey {}".format(policy_data['label'], policy_data['policy_encrypting_key']))
    ...
    data_source = Enrico.from_public_keys(
        ...
        verifying_key=data_source_metadata['data_source_ver_key'],
    )
